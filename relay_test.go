...
func BenchmarkRelay(b *testing.B) {
	...
	require.NoError(b, relay.ListenAndServe("127.0.0.1:0"), "relay listen failed")
	...
	for i := 0; i < numServers; i++ {
		...
		relay.ServiceHosts().Register("test", servers[i].PeerInfo().HostPort)
	}

	clients := make([]*Channel, numServers)
	for i := 0; i < numClients; i++ {
		clients[i] = testutils.NewClient(b, nil)
		clients[i].Peers().Add(relay.PeerInfo().HostPort)
	}
